  ...
  template: `
    <div class="page-center">
      <div class="card">
        ...
        <div class="card-sub">Enter the 6-digit code sent to <span style="color:var(--accent)">your email</span></div>

        <div class="alert alert-info">📧 OTP expires in 10 minutes</div>

        <div class="step-badges">
          <div class="step-badge done">① Email sent ✓</div>
          <div class="step-badge active">② Verify OTP ←</div>
          <div class="step-badge">③ New Password</div>
        </div>

        <div *ngIf="errorMsg" class="alert alert-error">{{ errorMsg }}</div>
        <div *ngIf="successMsg" class="alert alert-success">{{ successMsg }}</div>

        <form [formGroup]="form" (ngSubmit)="onVerifyResetOTP()">
          <div class="field">
...
export class ResetPasswordComponent implements OnInit {
  ...
  onVerifyResetOTP() {
    ...
    this.authService.verifyResetOtp(this.email, this.form.value.otp!).subscribe({
      next: () => {
        this.isLoading = false;
        // resetToken stored in authService state — NEVER in URL
  ...
  onResendOTP() {
    ...
    this.authService.resendOTP(this.email, 'PasswordReset').subscribe({
      next: () => {
